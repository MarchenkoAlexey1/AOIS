...
bool TruthTable::GetResultByIndex(std::vector<bool> index)
{
	for (int j = 0; j < pow(2, unique_vars.size()); j++) {
		int counter = 0;
		for (int i = 0; i < unique_vars.size(); i++) {
			if (truth_table[i][j] == index[i]) counter++;
		}
		if (counter == unique_vars.size()) return truth_table[unique_vars.size()][j];
	}
	return false;
}

int TruthTable::GetNumberOfZeros()
{
	int number_of_zeros = 0;

	for (int j = 0; j < pow(2, unique_vars.size()); j++) {
		if (truth_table[unique_vars.size()][j] == 0) number_of_zeros++;
	}

	return number_of_zeros;
}

int TruthTable::GetNumberOfUnits()
{
	int number_of_units = 0;

	for (int j = 0; j < pow(2, unique_vars.size()); j++) {
		if (truth_table[unique_vars.size()][j] == 1) number_of_units++;
	}

	return number_of_units;
}

std::string TruthTable::GetUniqueVars()
{
	return unique_vars;
}
